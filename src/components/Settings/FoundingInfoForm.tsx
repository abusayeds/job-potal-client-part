...
const FoundingInfoForm = () => {
  ...
  const visionRef = useRef<TTinyMcEditor>(null);
  ...
  const onFinish: FormProps<TUniObject>["onFinish"] = async (values) => {
    ...
    if (visionRef.current && typeof visionRef.current.getContent() === "string")
      values.companyVision = visionRef.current.getContent();
    try {
      await mutation({
        body: values,
        step: 2,
              ...
              rules={[
                {
                  validator() {
                    if (
                      !(
                        visionRef.current &&
                        typeof visionRef.current.getContent() === "string" &&
                        visionRef.current.getContent().length > 0
                      )
                    ) {
