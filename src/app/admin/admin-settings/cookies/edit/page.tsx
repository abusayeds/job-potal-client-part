...
const Page = () => {
  ...
  const { data } = useGetCookiesQuery({});

  const [editPrivacy, { isLoading }] = useEditCookiesMutation();
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = async (values) => {
    ...
    try {
      ...
      messageApi.open({
        key: "update",
        type: "success",
        content: "Cookies updated successfully!",
        duration: 2,
      });
      router.back();
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="space-y-5">
        <PageHeading title={"Edit Cookie Policy"} />
        {/* <LoaderWraperComp
            isLoading={isLoading}
            isError={isError}
            className={"h-[70vh]"}
              ...
              rules={[
                {
                  validator() {
                    if (
                      ...
                      return Promise.reject("Privact Policy is required!");
                    }
