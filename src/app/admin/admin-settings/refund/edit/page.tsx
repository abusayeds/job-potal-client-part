...
const Page = () => {
  ...
  const { data } = useGetRefundQuery({});
  // console.log(data?.data);

  const [editRefund, { isLoading }] = useEditRefundMutation();
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = async (values) => {
    ...
    try {
      ...
      messageApi.open({
        key: "update",
        type: "success",
        content: "Refund & Cancellation Policy updated successfully!",
        duration: 2,
      });
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="space-y-5">
        <PageHeading title={"Edit Refund & Cancellation Policy"} />
        {/* <LoaderWraperComp
            isLoading={isLoading}
            isError={isError}
            className={"h-[70vh]"}
